# -*- coding: utf-8 -*-
"""
Created on Mon Nov 27 13:21:11 2023
...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 26 22:22:48 2023

@author: abhi-

implemented rep counter
restructed pose_sequence logic when first pose is starting and ending pose 
    (for seamless repeition)
"""
...
baseline_pose_t1, baseline_connections_t1 = load_baseline_pose('half-kneel-archer-trimmed_pose_data.json',0)
baseline_pose_t2, baseline_connections_t2 = load_baseline_pose('half-kneel-archer-trimmed_pose_data.json',20)
baseline_pose_t3, baseline_connections_t3 = load_baseline_pose('half-kneel-archer-trimmed_pose_data.json',34)
baseline_pose_t4, baseline_connections_t4 = load_baseline_pose('half-kneel-archer-trimmed_pose_data.json',53)
baseline_pose_t5, baseline_connections_t5 = load_baseline_pose('half-kneel-archer-trimmed_pose_data.json',77)
baseline_pose_t6, baseline_connections_t6 = load_baseline_pose('half-kneel-archer-trimmed_pose_data.json',89)
baseline_pose_t7, baseline_connections_t7 = load_baseline_pose('half-kneel-archer-trimmed_pose_data.json',99)
...
pose_sequence = [
    ...
    (baseline_pose_t5, baseline_connections_t5),
    (baseline_pose_t6, baseline_connections_t6),
    (baseline_pose_t7, baseline_connections_t7)
]

# Initialize variables for pose sequence and timing
current_pose_index = 0
pose_hold_start = None
pose_hold_duration = .75  # seconds

match_threshold_percentage = 95  # Set the threshold for pose match

# Initialize the reps counter
reps_counter = 0
last_pose_index = 0  # To keep track of the last pose index
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        # Count the number of matching joints
        matching_joints = 0
        total_joints = len(new_limbs) * 2  # Each limb has two joints

        for limb_name, limb_joints in new_limbs.items():
            if compare_angles_for_limb(limb_joints[0], limb_joints[1], landmarks, pose_sequence[current_pose_index][0]):
                matching_joints += 2  # Increment by 2 for each limb

        # Calculate the percentage of matching joints
        matching_percentage = (matching_joints / total_joints) * 100

        # Check if the matching percentage meets the threshold
        if matching_percentage >= match_threshold_percentage:
            if pose_hold_start is None:
                ...
                next_pose_index = (current_pose_index + 1) % len(pose_sequence)
                if current_pose_index == len(pose_sequence) - 1 and last_pose_index != current_pose_index:
                    reps_counter += 1  # Increment reps counter when returning to the first pose
                #    if pose_hold_duration != 0:
                #        pose_hold_duration += -.25
        ...
        # Draw limbs with updated logic
        for limb_name, limb_joints in new_limbs.items():
            draw_limb(frame, limb_joints, landmarks, pose_sequence[current_pose_index][0])
    
    # Create the skeleton image (desired pose)
    desired_pose_frame = np.zeros_like(frame)
    draw_desired_pose(desired_pose_frame, *pose_sequence[current_pose_index])
    
    # Resize the skeleton image to fit in the corner
    thumbnail_size = (int(frame.shape[1] * 0.3), int(frame.shape[0] * 0.3))  # 30% of the original size
    desired_pose_thumbnail = cv2.resize(desired_pose_frame, thumbnail_size)
    
    # Overlay the skeleton image on the top left corner of the live feed
    ...
    cv2.putText(frame, f"Reps: {reps_counter}", (frame.shape[1] - 150, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    # Show the frame with user's pose and overlaid skeleton
